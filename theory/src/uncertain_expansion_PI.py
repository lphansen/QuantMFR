...
def uncertain_expansion(eq, ss, var_shape, args, gc, approach = '1', init_util = None, iter_tol = 1e-8, max_iter = 50, sloc = 0):
    """
    ...
    while error > iter_tol and i < max_iter:
        if approach == '1':
            J1_t, JX1_t, JX1_tp1, JX1_tp1_tilde, X1_tp1, X1_tp1_tilde, discount_adj = first_order_expansion_approach_1(df, util_sol, var_shape, H0_t, args, sloc)
            adj = compute_adj_approach_1(H_fun_list, ss, var_shape, util_sol, JX1_t, X1_tp1, H0_t, args)
            J2_t, JX2_t, JX2_tp1, JX2_tp1_tilde, X2_tp1, X2_tp1_tilde = second_order_expansion_approach_1(df, util_sol, X1_tp1, X1_tp1_tilde, JX1_t, JX1_tp1, JX1_tp1_tilde, adj, var_shape, discount_adj, sloc)
        elif approach == '2':
            J1_t, JX1_t, JX1_tp1, JX1_tp1_tilde, X1_tp1, X1_tp1_tilde = first_order_expansion_approach_2(df, util_sol, var_shape, H0_t, args, sloc)
...
def first_order_expansion_approach_1(df, util_sol, var_shape, H0, args, sloc):
    """
    ...
    ### The following steps are special for the permanent income model
    if np.linalg.matrix_rank(LHS_org) == len(RHS_org):
        D_org = np.linalg.solve(LHS_org, RHS_org)
    else:
        new_column = np.zeros((LHS_org.shape[0],1))
        new_column[sloc,0] = 1
        new_row = np.zeros((1, LHS_org.shape[1]+1))
        new_row[0,n_J] = 1
        LHS_org = np.vstack((np.hstack((LHS_org, new_column)),new_row))
        RHS_org = np.vstack((RHS_org, np.zeros(1)))
        
        D_org = np.linalg.solve(LHS_org, RHS_org)
        discount_adj_1 = np.float(D_org[-1])
        discount_adj = [discount_adj_1,None]
        D_org = D_org[:-1]
    ### Permanent income model special treatment ends here
    C = D_org[:n_J] - schur['N']@D_org[n_J:]
    # print(2, RHS_org)
    ...
    return J1_t, JX1_t, JX1_tp1, JX1_tp1_tilde, X1_tp1, X1_tp1_tilde, discount_adj
...
def second_order_expansion_approach_1(df, util_sol, X1_tp1, X1_tp1_tilde, JX1_t, JX1_tp1, JX1_tp1_tilde, adj, var_shape, discount_adj, sloc):
    """
    ...
    M_mat = form_M0(M_E_w, M_E_ww, X1_tp1_tilde, X1X1)
    ...
    var_G = LinQuadVar({'c':G[:,:1],
            'x':G[:,1:1+n_X],
            'xx':G[:,1+n_X:1+n_X+n_X**2]}, (n_J, n_X, n_W), False)
    res_G_next  = next_period(var_G, X1_tp1, None, X1X1)
    c_G_next    = res_G_next['c']
    w_G_next    = res_G_next['w']
    ww_G_next   = res_G_next['ww']
    cons_G_next = c_G_next+w_G_next@μ_0+ww_G_next@cal_E_ww(μ_0, cov_w)
    
    c_J_next    = np.zeros_like(X2_tp1['c'])
    c_J_next    = np.zeros((n_X,1))
    w_J_next    = X2_tp1['w']
    ww_J_next   = var_next['ww'] + ww_1
    cons_J_next = c_J_next+w_J_next@μ_0+ww_J_next@cal_E_ww(μ_0, cov_w)
        
    cons_1 = f_1_xtp1@np.block([[schur['N']@cons_J_next+cons_G_next],[cons_J_next]]) # Constant from the first term
    cons_2 = E_D2_coeff[:n_J,:1] # Constant from L_t
    R_1 = cons_1 + cons_2
        
    # Solve for R_2
    cons_1 = f_2_xtp1@np.block([[schur['N']@c_J_next+c_G_next],[c_J_next]])
    cons_2 = df['qq'][n_J:] + df['q'][n_J:].reshape(-1,1)
    R_2 = cons_1 + cons_2
    ...
    if np.linalg.matrix_rank(LHS_const_term) == len(LHS_const_term):
        ...
        G[:,:1] = D[:n_J] - schur['N'] @D[n_J:]

    X2_tp1 = LinQuadVar({'x2': X1_tp1['x'],
                'xx': var_next['xx'] + xx_1 + xx_2,
                'xw': var_next['xw'] + xw_1,
                'ww': var_next['ww'] + ww_1,
                'x': var_next['x'] + x_1 + x_2,
                'w': var_next['w'] + w_1,
                'c': D[n_J:] - X1_tp1['x']@D[n_J:]},
                (n_X, n_X, n_W), False)

    ### Permanent income model special treatment ends here
    X2_tp1_tilde = update_state_evolution0(X1_tp1, X2_tp1, μ_0, var_shape)
...
#     Wtp1 = LinQuadVar({'w': np.eye(n_W)}, (n_W, n_X, n_W), False)
#     X2_tp1, G = solve_second_state(X1_tp1, JX1_tp1, Wtp1)
#     X2_tp1_tilde = update_state_evolution0(X1_tp1, X2_tp1, μ_0, var_shape)
...
def first_order_expansion_approach_2(df, util_sol, var_shape, H0, args, sloc):
    """
...
def update_state_evolution0(X1_tp1, X2_tp1, μ_0, var_shape):
    
    n_Y, n_Z, n_W = var_shape
    ψ_x, ψ_w, ψ_q, ψ_x2, ψ_xx, ψ_xw, ψ_xq, ψ_ww, ψ_wq, ψ_qq = extract_ψ(X1_tp1, X2_tp1)

    ψ_tilde_x = ψ_x 
    ψ_tilde_w = ψ_w
    ψ_tilde_q = ψ_q + ψ_w@μ_0
    Z1_tp1 = LinQuadVar({'x': ψ_tilde_x, 'w': ψ_tilde_w, 'c': ψ_tilde_q}, (n_Z, n_Z, n_W), False)
    # Second order
    Ix = np.eye(n_Z)
    ψ_tilde_xq = ψ_xq + ψ_xw@np.kron(Ix,μ_0) 
    ψ_tilde_wq = ψ_wq + ψ_ww@np.kron(np.eye(n_W),μ_0) + ψ_ww@np.kron(μ_0,np.eye(n_W))
    ψ_tilde_qq = ψ_qq + ψ_ww@np.kron(μ_0, μ_0) + ψ_wq@μ_0
    ψ_tilde_xx = ψ_xx 
    ψ_tilde_xw = ψ_xw
    ψ_tilde_ww = ψ_ww
...
def update_state_evolution(X1_tp1, X2_tp1, N_cm, var_shape):
